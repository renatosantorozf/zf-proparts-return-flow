  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (parsed.rows.length === 0) {
        setErrorMsg('Nenhuma linha válida encontrada na planilha.')
        setState('error')
        return
      }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="card p-6 space-y-4">
        ...
        {state === 'error' && (
          <div className="space-y-3">
            ...
            <button onClick={reset} className="btn-secondary text-sm">Tentar novamente</button>
          </div>
        )}
      ...
      <div className="card p-6 opacity-50">
        ...
        <p className="text-sm text-gray-400">Configuração de dias úteis por coluna do Kanban — disponível na Sprint 2.</p>
      </div>
    </div>
  )
}
